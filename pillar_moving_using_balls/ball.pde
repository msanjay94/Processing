class Ball
{
  ...
  void check(Pillar p) {
    if (this.loc.x>=(p.loc.x-this.radius) && this.loc.x<=(p.loc.x+this.radius+p.rectWidth)) {
      if (this.loc.y>=(p.loc.y-this.radius) && this.loc.y<=(p.loc.y+p.rectHeight+this.radius )) {
        javax.swing.JOptionPane.showMessageDialog(frame, "You lose\nScore:"+score/30);
        System.exit(0);
      } else if (this.loc.y>=(p.loc.y-this.radius+p.rectHeight+150) && this.loc.y<=height+this.radius) {
        javax.swing.JOptionPane.showMessageDialog(frame, "You lose\nScore : "+score/30);
        System.exit(0);
      } else {
